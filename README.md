...
##  rfid_protection.py — Protection Module

A dedicated module you can import into the main script:

```python
#!/usr/bin/env python3
"""
rfid_protection.py — Safety & Protection Module
for the RFID Magic Card UID Reader & Writer.

Provides:
  • UID / BCC validation
  • Safe-write guard (read-only mode enforcement)
  • Card type detection before any write
  • Duplicate-card debounce
  • Auth state cleanup
  • Rate limiting for key attempts
"""

import time


class RFIDProtection:
    """Central protection layer for all card operations."""

    def __init__(self, real_write_enabled=True, max_key_attempts=7,
                 cooldown_seconds=0.5):
        self.real_write_enabled = real_write_enabled
